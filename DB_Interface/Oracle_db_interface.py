...
class Database:
    ...
    def __init__(self, config_file):
        ...
        self.__database_hostname = self.__config_parser.get(const.CONFIG_META_HEADER_ORACLE, const.CONFIG_META_DATA_DB_HOSTNAME)
        self.__database_username = self.__config_parser.get(const.CONFIG_META_HEADER_ORACLE, const.CONFIG_META_DATA_DB_USERNAME)
        password = self.__config_parser.get(const.CONFIG_META_HEADER_ORACLE, const.CONFIG_META_DATA_DB_PASSWORD)
        try:
            self.__database_password = (base64.b64decode(password)).decode()
        except:
            self.__database_password = password
        self.__database_port = self.__config_parser.get(const.CONFIG_META_HEADER_ORACLE, const.CONFIG_META_DATA_DB_PORT)
        self.__database_name = self.__config_parser.get(const.CONFIG_META_HEADER_ORACLE, const.CONFIG_META_DATA_DB_NAME)
        self.__host_domain = self.__config_parser.get(const.CONFIG_META_HEADER_ORACLE, const.CONFIG_META_DATA_HOST_DOMAIN)
        self.__database_dsn = db_interface.makedsn(self.__database_hostname, self.__database_port, self.__database_name)
    ...
    def connect_database(self):
        try:
            connection = db_interface.connect(self.__database_username, self.__database_password, self.__database_dsn)
